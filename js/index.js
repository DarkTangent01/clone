...
$(function () {
	...
	$(".canCheck").click(function () {
		...
		if ($(this).find("input").is(":checked")) {
			$(
				".checkable[data-trigger='" + $(this).find("input").attr("class") + "']"
			).attr("data-checked", "true");
			$("[" + $(this).find("input").attr("class") + "]").attr(
	...
	function LockScreenCheckPass() {
		...
		if (password === "123") {
			...
			setTimeout(function () {
				$(".lockScreenLogIn").addClass("wrong").removeClass("wait");
   ...
   $('.taskbar__item[data-window="' + appName + '"]').addClass('taskbar__item--active');
	...
	function createWindow(data, content, width, height, tmp, bD) {
		...
		$(".window[data-window='" + d + "']")
			...
			.mousedown(function () {
				...
				setTimeout(function () {
					$(parentWindow).hide().removeClass("window--closing");
	...
	function openApp(e) {
		...
		if (
			($(this).data("trigger") !== null && targetWindow == null) ||
			($(this).data("trigger") == null && targetWindow !== null)
		) {
			appMenuClose();
		}
...
$(".audio .cancel").mousedown(function () {
	$(".audio").removeClass("recorded").removeClass("recording");
...
$(".audio .trigger").mousedown(function () {
	...
	if ($(".audio").hasClass("recorded")) {
		...
		if ($(".audio").hasClass("recording")) {
			...
			$(".audio").addClass("recording").addClass("minLen");
			$(".audioMargin").toggleClass("recording");
...
function sysIn(e) {
	return e
		? $(".console-input").val().split(" ").shift()
		: $(".console-input")
				.val()
				.split(" ")
				.slice(1, $(".console-input").val().split(" ").length);
}
...
var cmds = {
	...
	edit: function (args) {
		...
		output([
			"Opening " +
				(debug ? "debug view " : "script ") +
				(tab ? "on a new tab" : "") +
				"..."
		]);
		window.open(
			debug
				? "https://codepen.io/Varo/debug/BypqMJ"
	...
	help: function (a) {
		if (a == "") {
			...
			if (Object.keys(cmds).indexOf(sysIn().shift()) !== -1) {
				...
				output([
					"<div class='err'>'" + sysIn().shift() + "' is not a command</div>"
				]);
			}
		}
	}
...
$(".console-input").keydown(function (event) {
	...
	if (
		...
		if (typeof cmds[cmd] == "function") {
			...
			output([
				"<div class='err'>Command not found: '" + cmd + "'</div>",
